...
class WeatherApp:
    ...
    def saveTree(self, filename, clf):
        joblib.dump(clf, filename)

    def loadTree(self, filename):
        return joblib.load(filename)

    def printDecisionTree(self, clf, outputfile, features_names, label_names):
        dot_data = StringIO()
    ...
    def calculateDistance(self, lat1, long1, lat2, long2):#haversine formula
        r = 6371 * pow(10, 3)
    ...
    def makeSlidingWindow(self, arr, windowLength):
        arr2 = []
        i = 0
        j = 0
        while i + j < len(arr):
    ...
    def calcEuclideanDist(self, arr2d1, arr2d2):
        a = numpy.array(arr2d1)
        b = numpy.array(arr2d2)
        return numpy.linalg.norm(a)

    def calcMean(self, arr):
        sum = 0
        for i in range(len(arr)):
            sum = sum + arr[i]
        return sum / len(arr)

    def slidingWindowWeather(self, CD, PD, days):
        W = self.makeSlidingWindow(PD, days)
        ED = []
        for i in range(len(W)):
            ED.append(self.calcEuclideanDist(W[i], CD))
        ...
        for i in range(len(CD[0])):
            ...
            m1 = self.calcMean(variationCDVector)
            m2 = self.calcMean(variationPDVector)
            V = (m1 + m2) / 2
            predicted[i] = predicted[i] + V
        return predicted

    def getData(self, mainQueue, filename, wtParams):
        times = []
        forecasts = []
        weatherData = []
    ...
    def mlTrain(self, mainQueue, weatherDataLoc, forecasts):
        clf = tree.DecisionTreeClassifier()
        clf = clf.fit(weatherDataLoc, forecasts)
        mainQueue.put(clf)

    def nwpModel(self, mainQueue, time1, 
        ...
        distance, angle = self.calculateDistance(latitude1, longitude1, latitude2, longitude2)
    ...
    def slidingWindowModel(self, mainQueue, currentDate, times, weatherDataLoc):
        CD = []
        PD = []
        ...
        slidingWindowPrediction =  self.slidingWindowWeather(CD, PD, 7)
        mainQueue.put(slidingWindowPrediction)

    def runModel(self, startDate, city, lat1, long1, lat2, long2, lat3, long3, d1, d2, d3):
        predictions = []
        times = d1[0]
        forecasts = d1[1]
        ...
        #run models
        q = queue.Queue()
        t0 = Thread(self.mlTrain(q, weatherDataLoc1, forecasts))
        t0.start()
        ...
        t1 = Thread(self.nwpModel(q, currentDate, 
                        lat1, long1, 
                        lat2, long2,
        ...
        t2 = Thread(self.nwpModel(q, currentDate, 
                        lat1, long1, 
                        lat3, long3,
                        weatherDataLoc1[dateIndex][5], 
                        weatherDataLoc1[dateIndex][6], 
        ...
        t3 = Thread(self.slidingWindowModel(q, currentDate, times, weatherDataLoc1))
        t1.start()
        t2.start()
        t3.start()
        t0.join()
    ...
    def run(self, startDate):
        ...
        for k in range(len(cities)):
            t = Thread(self.getData(q, './input/weather_data_'+cities[k]['city']+'.csv', self.weatherParams)) 
            t.start()
            threads.append(t)
        ...
        with concurrent.futures.ProcessPoolExecutor() as executer:
            ...
            for f in concurrent.futures.as_completed(results):
                results_arr.append(f.result())
        return results_arr

# if __name__ == '__main__':
#     w = WeatherApp(['precipIntensity', 'precipProbability', 'temperature', 'humidity', 'pressure', 'windSpeed', 'windBearing'])
#     r = w.run()
#     print(r)